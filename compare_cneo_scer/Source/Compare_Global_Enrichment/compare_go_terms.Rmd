...
list_of_selected_go_terms <- read_tsv( file.path( compare_data_dir, "Selected_GO_Terms/list_of_selected_GO_terms.txt"), 
                                       col_names = TRUE) %>%
                             dplyr::select(-term)
...
data_plot_odds_ratio <-  data_plot_odds_ratio_temp  %>%
  ...
  dplyr::rename( p.adjusted = "bh") %>%
  filter ( !is.na(x_labels) & x_labels != "NA") %>%
...
max_odds_ratio_score <- subset_to_plot %>%   
        filter ( is.finite(Odds_Ratio_Score ) ) %>%
        summarise( max_odds_ratio_score =  max( Odds_Ratio_Score))  %>%
        pull(max_odds_ratio_score )  %>%
        ceiling
  
subset_to_plot_clean_max <- subset_to_plot %>%
                            mutate(Odds_Ratio_Score = ifelse( is.finite(Odds_Ratio_Score), Odds_Ratio_Score, max_odds_ratio_score)) %>%
                            arrange( groups, ordering)
```


```{r}
odds_ratio_ggplot <- subset_to_plot_clean_max  %>%
    ggplot( aes(  x_labels, term,
                  color = direction,
                  shape = direction, 
                  fill = Odds_Ratio_Score, 
                  size = -log10(p.adjusted)  )) +
    geom_point() + 
    scale_shape_manual(values = c(24,25), name = "Direction") +
    scale_fill_gradient2(high = "coral3", low = "springgreen4", mid = "white", name=str_wrap("Odds Ratio Score", width=15)) + 
    scale_color_manual(values = c( "coral3", "springgreen4"), name = "Direction") +
    ...
    facet_grid( groups ~ . , drop = TRUE, scales = "free", space = "free", 
...
```{r}
ggsave ( filename = file.path( compare_go_terms_results_dir, "compare_s_cerevisiae_c_neoformans_go_terms.png"),  width= 10,  height = 11 )
ggsave ( filename = file.path( compare_go_terms_results_dir, "compare_s_cerevisiae_c_neoformans_go_terms.pdf"),  width= 10,  height = 11 )
ggsave ( filename = file.path( compare_go_terms_results_dir, "compare_s_cerevisiae_c_neoformans_go_terms.tiff"), width= 10,  height = 11 )
...
Inkscape scaling 70% of original
