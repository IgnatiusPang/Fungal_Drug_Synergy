...
compare_A_CA_ggplot <- compare_A_CA_orthologs_clean %>%
  ...
  mutate( Comparison = "AMB only treatment vs. control" ) %>%
  ggplot( aes( logFC.cneo, logFC.scer)) +
  geom_point( aes(col=is_significant))  + 
  ...
  ylim( -4, 2.8) + 
  # theme(legend.position = "none")  + # Remove the legned
  theme(legend.text.align = 0)  +  # Align legend text to the left,  
  facet_grid( . ~  Comparison, labeller = label_wrap_gen() )  

compare_A_CA_ggplot

ggsave( filename = file.path(compare_orthologs_results_dir, "compare_A_CA_ggplot.png"), plot=compare_A_CA_ggplot)
ggsave( filename = file.path(compare_orthologs_results_dir, "compare_A_CA_ggplot.tiff"), plot=compare_A_CA_ggplot)
ggsave( filename = file.path(compare_orthologs_results_dir, "compare_A_CA_ggplot.pdf"), plot=compare_A_CA_ggplot, 
        width= 5 ,  height = 3.3)
```
...
cat("Compare A and CA log FC in S. cerevisiae and C. neoformans, Pearson's test")
cor.test( compare_A_CA_orthologs$logFC.cneo, 
          compare_A_CA_orthologs$logFC.scer, method="pearson" )
sink()



sink( file.path(compare_orthologs_results_dir, "compare_A_CA_pearson_sig_genes.txt"))
cat("Compare A and CA significant genes log FC for sig genes in S. cerevisiae and C. neoformans, Pearson's test")
...
compare_AL_CAL_ggplot <- compare_AL_CAL_orthologs_clean %>%
  ...
  mutate( Comparison = "AMB-LF treatment vs. control" ) %>%
  ggplot( aes( logFC.cneo, logFC.scer)) +
  geom_point( aes(col=is_significant))  + 
  geom_smooth(aes(group = 1), method = "lm") +
...
ggsave( filename = file.path(compare_orthologs_results_dir, "compare_AL_CAL_ggplot.tiff"), plot=compare_AL_CAL_ggplot)
ggsave( filename = file.path(compare_orthologs_results_dir, "compare_AL_CAL_ggplot.pdf"), plot=compare_AL_CAL_ggplot, 
        width= 5 ,  height = 3.3 )
```
...
cat("Compare AL and CAL log FC in S. cerevisiae and C. neoformans, Pearson's test")
cor.test( compare_AL_CAL_orthologs$logFC.cneo, 
          compare_AL_CAL_orthologs$logFC.scer, method="pearson" )
sink()



sink( file.path(compare_orthologs_results_dir, "compare_AL_CAL_pearson_sig_genes.txt"))
...
# plot_grid(compare_A_CA_ggplot, compare_AL_CAL_ggplot, labels = c('(A)', '(B)'), rel_widths = c(0.35, 0.55))
...
compare_A_CA_for_merge <-  compare_A_CA_orthologs_clean %>% mutate( Comparison = "AMB only treatment vs. control" )
compare_AL_CAL_for_merge <-  compare_AL_CAL_orthologs_clean %>% mutate( Comparison = "AMB-LF treatment vs. control" )

merged_data <- compare_A_CA_for_merge %>%
               bind_rows( compare_AL_CAL_for_merge) %>%
               mutate( Comparison = factor( Comparison, levels = c( "AMB only treatment vs. control", 
                                                                    "AMB-LF treatment vs. control")))

merged_ggplot <- merged_data %>%
  dplyr::mutate( is_significant = factor( case_when( FDR.cneo < 0.05  & FDR.scer < 0.05  ~ "Both significant",
                                    FDR.cneo < 0.05  & FDR.scer >= 0.05 ~ "C. neoformans Only",
                                    FDR.cneo >= 0.05 & FDR.scer < 0.05  ~ "S. cerevisiae Only",
                                    TRUE ~ "Not sigificant" ), 
...
ggsave( filename = file.path(compare_orthologs_results_dir, "merged_ggplot.png"), plot=merged_ggplot, width= 7 ,  height = 3 )
ggsave( filename = file.path(compare_orthologs_results_dir, "merged_ggplot.tiff"), plot=merged_ggplot, width= 7 ,  height = 3 )
ggsave( filename = file.path(compare_orthologs_results_dir, "merged_ggplot.pdf"), plot=merged_ggplot, width= 7 ,  height = 3 )
ggsave( filename = file.path(compare_orthologs_results_dir, "merged_ggplot.svg"), plot=merged_ggplot, width= 7 ,  height = 3 )
