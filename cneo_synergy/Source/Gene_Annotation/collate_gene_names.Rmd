...
cneo_part1 <- read_tsv(  file.path( data_dir, "Broad_C_neoformans_Mar_2014", 
                                    "cryptococcus_neoformans_var._grubii_h99__cna3__3_genome_summary_per_gene.txt" )  )
cneo_part2 <- read_tsv( file.path( data_dir, "Broad_C_neoformans_Mar_2014", 
                                   "cryptococcus_neoformans_var._grubii_h99_mitochondria_2_genome_summary_per_gene.txt" )  )
...
```{r}
cneo_uniprot_annot <- read.table( file.path(data_dir, 
                                            "Uniprot/cneo/uniprot-cryptococcus+neoformans+h99.tab") , 
                                  sep="\t", stringsAsFactors = FALSE, quote = "", header=TRUE ) 
scer_uniprot_annot <- read.table( file.path(data_dir, 
                                            "Uniprot/scer/uniprot-proteome_UP000002311.tab") , 
                                  sep="\t", stringsAsFactors = FALSE, quote = "", header=TRUE  ) 
...
write.table( cneo_uniprot_acc_to_orf_id, file.path( data_dir, "Gene_Annotation/cneo_uniprot_acc_to_orf_id.tsv" ))
