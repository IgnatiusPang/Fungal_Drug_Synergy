...
old_bp  <- read.table( file.path( data_dir, "Gene_Ontology_Annotation/old/cneo_goframeData_bp.tab" ), header=TRUE )
old_cc  <- read.table( file.path( data_dir, "Gene_Ontology_Annotation/old/cneo_goframeData_cc.tab" ), header=TRUE )
old_mf  <- read.table( file.path( data_dir, "Gene_Ontology_Annotation/old/cneo_goframeData_mf.tab" ), header=TRUE )

```

## QuickGO
```{r}
quickgo <- read.table(file.path( data_dir, "QuickGO/QuickGO-annotations-1554894162403-20190410.tsv"  ), sep="\t",
...
fungidb <- read.table(file.path( data_dir, "FungiDB/GenesByTaxon_GOTerms.csv"  ), sep=",",
                       stringsAsFactors = FALSE, header=TRUE, quote = "\"", strip.white=TRUE)
...
aspgd_go <- read.table ( file.path( data_dir, "Gene_Ontology_Raw/Species_annotation/New/aspgd.gaf" ) , sep="\t",
...
pombase_go <- read.table ( file.path( data_dir, "Gene_Ontology_Raw/Species_annotation/New/pombase.gaf" ) , sep="\t",
                       stringsAsFactors = FALSE, header=FALSE, strip.white=TRUE, quote="", comment.char="!"  ) 
...
sgd_go <- read.table ( file.path( data_dir, "Gene_Ontology_Raw/Species_annotation/New/sgd.gaf" ) , sep="\t",
                       stringsAsFactors = FALSE, header=FALSE, strip.white=TRUE, quote="", comment.char="!"   ) 
...
## Mushroom
avar_go <- read.table ( file.path( data_dir, "Gene_Ontology_Raw/Species_annotation/New/Abisporus_varbisporusH97.v2.goinfo_FilteredModels3.tab" ) ,
...
blast2go   <- read.table( file.path(results_dir, "Blast2GO/blast2go_gaf.txt") , sep="\t",
...
write.table(all_tables_ancestors_cleaned,  file.path( data_dir, "Gene_Ontology_Annotation", 
...
# 
# 
# all_tables_ancestors_cleaned_old <- read_tsv ( file.path(data_dir, "Gene_Ontology_Annotation/Ancestor_Cleaned/old/all_gene_ontology_db_merged.tab")) %>%
#                                     distinct()
# 
# 
...
purrr::walk2(list_of_tables, list_of_output_files, 
             function( x, y){ write.table( x, 
                                           file.path( data_dir, "Gene_Ontology_Annotation", y ),
                                           sep="\t", row.names = FALSE, col.names = TRUE, quote=FALSE) } )
