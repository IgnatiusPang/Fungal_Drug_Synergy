...
vroom::vroom_write(selected_by_go_terms_soms, path=file.path(validation_results_dir, "selected_by_go_terms_soms.tsv" ))
```
...
genes_double_fold_change_wrt_AMB_only <-  de_genes_al %>%
  ...
  filter( !is.na( blast2go_annotation )) ## No Blast2GO annotation, too hard to characterize using bioinformatics tools


vroom::vroom_write(genes_double_fold_change_wrt_AMB_only, path=file.path(validation_results_dir, "genes_double_fold_change_wrt_AMB_only.tsv" ))
