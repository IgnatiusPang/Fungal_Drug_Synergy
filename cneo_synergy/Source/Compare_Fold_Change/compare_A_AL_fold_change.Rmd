...
all_genes_specific_to_A <- compare_fold_change %>% 
  filter ( coarse_groups == "AMB only") %>%
  arrange( desc(logFC.A )) %>%
  mutate( rank_from_biggest_increase = row_number()) %>%
  arrange( logFC.A ) %>%
  mutate( rank_from_biggest_decrease = row_number()) %>%
  left_join( gene_annotation, by=c("genes" = "orf_id")) %>%
  dplyr::select( -logFC.AL, -FDR.AL  )

write.table( all_genes_specific_to_A, file.path( results_dir, "Compare_Fold_Change", "all_genes_specific_to_A.tab"  ),
             row.names = FALSE, quote = FALSE, sep="\t") 
