...
specific_variation <-   "Remove_5a" # "All_Samples" #

choose_k_results_dir <-  cneo_edger_ruvs_ql_results_dir
...
num_sde_genes_table <- dplyr::mutate( num_sde_genes_table, 
                    ...
                    filter ( Comparison != "compare_AL_A") %>%
                    dplyr::mutate( Comparison = case_when ( Comparison == "compare_A" ~ "AMB treatment",
                                                            Comparison == "compare_AL" ~ "AMB-LF treatment",
                                                            TRUE ~ NA_character_
                                                            ))
...
my_graph <- num_sde_genes_table %>%
      ...
      ylab("No. of genes") +
      xlab("Number of unwanted factors (k)") + 
      guides(color=guide_legend(title="Direction"))

my_graph


ggsave( file.path( compare_k_results_output_dir, "compare_num_sde_genes.png" ), width = 6, height =  3 )
ggsave( file.path( compare_k_results_output_dir, "compare_num_sde_genes.pdf" ), width = 6, height =  3 )
...
ggsave( file.path( compare_k_results_output_dir, "compare_num_sde_genes_up_and_down.png" ), width = 6, height =  3 ) 
...
p_values_table <- dplyr::mutate( p_values_table, 
									  k=purrr::map_int( k, function(x) { strsplit(x, "_")[[1]][2] %>% as.integer() }  )) %>%
                  filter ( expt_label != "compare_AL_A")
