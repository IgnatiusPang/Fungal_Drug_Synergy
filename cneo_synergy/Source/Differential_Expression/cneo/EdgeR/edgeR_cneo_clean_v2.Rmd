...
edger_gene_lists_dir <- file.path( choose_k_results_dir, "Gene_Lists")
create_dir_if_not_exists( edger_gene_lists_dir)
...
plot_PCA_RLE <- function ( set_before,  title_for_before_ruv, expt_group_list, treatment_type_list, results_dir, output_file_name ) {
...
y <- estimateGLMCommonDisp(y, design_for_ruv, verbose=TRUE)
...
if ( is_quasi_likelihood_test == TRUE) {
	cneo_fit <- glmQLFit( y , design_for_ruv)
	lrt <- glmQLFTest(cneo_fit, coef=2:ncol(cneo_fit$design))

} else {
	cneo_fit <- glmFit( y , design_for_ruv)
...
if ( is_quasi_likelihood_test == TRUE) {
	...
	fit_by_group <- glmQLFit(cneo_dgelist, design_by_group)

} else {
	
	fit_by_group <- glmFit(cneo_dgelist, design_by_group)
...
get_top_tags_baseed_on_group <- function ( group,  fit_by_group ) {
	...
	cex.main = 1)
	  ...
	  pdf(file.path(edger_gene_lists_dir, paste( group, "_", "plotSmear.pdf", sep="")))
    ...
    ### plot log-fold change against log-counts per million, with DE genes highlighted
	  png(file.path(edger_gene_lists_dir, paste( group, "_", "plotSmear.png", sep="")))
    plotSmear(lrt, de.tags=top %>% filter( FDR < de_genes_fdr_cutoff) %>% pull(genes), main=group)
    abline(h=c(-1, 1), col="blue")
    dev.off()
...
merge_with_gene_annotation <- function ( input_table, gene_annotation) {
	...
	return_table <- input_table %>%
					dplyr::left_join( gene_annotation, by=c("genes" = "orf_id")) %>%
					dplyr::distinct()
...
purrr::walk2( list_of_up_and_down_genes, names(list_of_up_and_down_genes),    
              function(x, y ) {  write.table( data.frame(orf_id= x),    
