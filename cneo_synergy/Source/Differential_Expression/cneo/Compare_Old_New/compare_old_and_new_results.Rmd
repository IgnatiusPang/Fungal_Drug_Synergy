...
```{r}
diff_exp_results_dir <- "/home/ignatius/PostDoc/2019/cneo_synergy/Results/Differential_Expression"
data_set_to_use <- "To_Use_k_0_Remove_5a"
...
load( file.path( diff_exp_results_dir, "Old_Results/20150609/edgeR_cneo_compare_delete_sample_5A.Rdata") )

compare_A_CA_old   <- topTags(cneo.lrt_compare_del_A, n=7000)  %>% as.data.frame %>% arrange( desc(logFC) )
compare_AL_CAL_old <- topTags(cneo.lrt_compare_del_AL, n=7000) %>% as.data.frame %>% arrange( desc(logFC) )


write.table(  compare_A_CA_old, 
           file.path( diff_exp_results_dir, "Old_Results/20150609/cneo_delete_sample_compare_A_fc_p_values.tab" ),
           sep="\t", row.names = FALSE, quote = FALSE, na="")
write.table(  compare_AL_CAL_old, 
           file.path( diff_exp_results_dir, "Old_Results/20150609/cneo_delete_sample_compare_AL_fc_p_values.tab" ),
           sep="\t", row.names = FALSE, quote = FALSE, na="")

# compare_A_CA_old   <- read_tsv( file.path( diff_exp_results_dir, "Old_Results/20150609/cneo_delete_sample_compare_A_fc_p_values.tab" ) )
# compare_AL_CAL_old <- read_tsv( file.path( diff_exp_results_dir, "Old_Results/20150609/cneo_delete_sample_compare_AL_fc_p_values.tab" )  )

compare_A_CA_new   <- read_tsv( file.path( diff_exp_results_dir, "EdgeR_RUVs_QL/cneo", data_set_to_use, "Gene_Lists/compare_A.tab" )  )
compare_AL_CAL_new <- read_tsv( file.path( diff_exp_results_dir, "EdgeR_RUVs_QL/cneo", data_set_to_use, "Gene_Lists/compare_AL.tab" )  )
...
compare_A_CA_num_sde_genes <-  compare_A_CA_old_new %>% 
  ...
  group_by( Count_New_SDE_Genes, Count_Old_SDE_Genes) %>%
  summarise( counts =n())
