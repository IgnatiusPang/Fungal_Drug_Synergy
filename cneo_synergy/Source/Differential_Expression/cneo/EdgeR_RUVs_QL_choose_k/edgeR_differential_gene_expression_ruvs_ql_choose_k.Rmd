...
edger_ruvs_gene_lists_dir <- file.path( choose_k_results_directory, "Gene_Lists")
create_directory_if_not_exists( edger_ruvs_gene_lists_dir)
...
```{r}
cneo_dgelist <- readRDS( file=file.path( choose_k_results_directory, "cneo_deg_ruv.Rdata") ) 
...
ruvs_design <- readRDS( file=file.path( choose_k_results_directory, "ruvs_design_matrix.Rdata") ) 

design_matrix <- read.table(cneo_design_matrix_file, header=TRUE, sep="\t", stringsAsFactors = TRUE)

biological_replicates <- c( "1", "2", "3" )
names( biological_replicates) <- as.character( sort( unique(  design_matrix$Date ) ))

design_matrix <- design_matrix %>%
				 dplyr::mutate( Biological_Replicate = biological_replicates[ as.character(Date)] )
...
design_matrix <- design_matrix[colnames(cneo_dgelist$E),]
colnames(cneo_dgelist$E) ==rownames(design_matrix)
