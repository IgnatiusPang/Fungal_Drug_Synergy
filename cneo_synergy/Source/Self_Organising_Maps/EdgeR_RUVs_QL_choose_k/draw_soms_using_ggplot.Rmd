...
compare_A_clusters <- compare_A_soms %>%
                      ...
                      inner_join( compare_A_norm_counts, by = c("id_a" = "orf_id", "sample_name" = "samples")) %>%
...
compare_AL_clusters <- compare_AL_soms %>%
                      mutate( sample_name = purrr::map_chr( x, ~convert_num_to_sample_name[.]   )  ) %>%
                      inner_join( compare_AL_norm_counts, by = c("id_a" = "orf_id", "sample_name" = "samples")) %>%
                      inner_join( cluster_id_to_soms_coord, by =c("cluster" = "cluster_id"))
...
compare_A_ggplot <- compare_A_clusters %>%
  ...
  ggplot( aes( sample_name, y,  col=cluster, group=id_a )) +
  geom_line() +
...
ggsave(filename = file.path(soms_results_dir, "compare_A/5x5/ggplot_SOMs_5_by_5.tiff"), plot=compare_A_ggplot, width = 6,  height =6.1  )
ggsave(filename = file.path(soms_results_dir, "compare_A/5x5/ggplot_SOMs_5_by_5.png"), plot=compare_A_ggplot, width = 6,  height =6.1  )
...
ggsave(filename = file.path(soms_results_dir, "compare_AL/5x5/ggplot_SOMs_5_by_5.tiff"), plot=compare_AL_ggplot, width = 6,  height =6.1  )
ggsave(filename = file.path(soms_results_dir, "compare_AL/5x5/ggplot_SOMs_5_by_5.png"), plot=compare_AL_ggplot, width = 6,  height =6.1  )

```


## Testing and checking with Cluster 19
```{r}
