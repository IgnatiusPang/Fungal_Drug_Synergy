...
go_terms_enrichment_adj_p_value_cutoff <- 1
...
```{r run6}
gene_annotation_table <- read_tsv( file.path( data_dir, "Gene_Annotation/cneo_gene_annotation_summary.txt"  ) )
...
run_one_soms_and_go_experiment_large_scale <- function( data_matrix, list_of_top_tags_table, 
		...
		go_enrichment_results <- run_soms_enrichment_tests( cluster_for_each_gene, 
																	 ...
																	 ontologies_list ,
																	 pvalueCutoff = go_terms_enrichment_adj_p_value_cutoff,
...
updated_all_time_points <-  purrr::cross_df( input_for_cross_df) %>%
						dplyr::mutate( sample_name = paste0( replicate_number, types_of_treatments )) %>%
						dplyr::select ( one_of ( c( "sample_name"))) %>%
						as.data.frame() %>%
						t() %>%
						as.vector()
...
list_of_clusters <- purrr::map ( list_of_clusters, 
                                 function(my_table) { dplyr::mutate( my_table,  
