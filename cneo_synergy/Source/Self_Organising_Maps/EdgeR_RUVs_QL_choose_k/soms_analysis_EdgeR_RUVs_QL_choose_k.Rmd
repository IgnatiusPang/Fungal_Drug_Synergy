...
source_rmd_file( file.path(source_directory, "Functional_Enrichment/Common/functional_enrichment_functions.Rmd" ) )
source_rmd_file( file.path(source_directory, "Self_Organising_Maps/Common/soms_functions.Rmd" ) )
...
EdgeR_RUVs_QL_de_genes_dir <- file.path( results_directory, "Differential_Expression/EdgeR_RUVs_QL/cneo", results_to_use_dir)
...
EdgeR_RUVs_QL_soms_results_dir  <- file.path( results_directory, "Self_Organising_Maps/EdgeR_RUVs_QL", results_to_use_dir, "Condition_on_child_terms_false") 

if( condition_on_child_terms_param == TRUE ) {
  
  EdgeR_RUVs_QL_soms_results_dir  <- file.path( results_directory, "Self_Organising_Maps/EdgeR_RUVs_QL", results_to_use_dir, "Condition_on_child_terms_true") 

}

create_directory_if_not_exists(EdgeR_RUVs_QL_soms_results_dir)
```
...
go_dictionary <- get_crypto_go_dictionary_of_choice  ( "Cryptococcus neoformans H99", gene_ontology_directory, 
									  bp_file_name="cneo_goframeData_bp.tab", 
									  mf_file_name="cneo_goframeData_mf.tab", 
									  cc_file_name="cneo_goframeData_cc.tab"  ) 

gene_annotation <- read.table ( file.path( data_directory, 'Gene_Annotation/cneo_gene_annotation_summary.txt' ) , header=TRUE, sep="\t", quote="", stringsAsFactors = FALSE)


universe <- unique( gene_annotation[, "orf_id"])

```

## Read in the gene annotations
```{r run6}
gene_annotation_table <- read_tsv( file.path( data_directory, "Gene_Annotation/cneo_gene_annotation_summary.txt"  ) )
...
if( results_to_use_dir == "To_Use_k_0_Remove_5a"  ) {
...
run_one_soms_and_go_experiment_large_scale <- function( data_matrix, list_of_top_tags_table, 
														...
														ontologies_list = c("BP", "CC", "MF"), results_directory) { 
		
		counts_data_matrix <- data_matrix[list_of_genes,]
		
		soms_results_directory <-  file.path(results_directory, expt_label,
		 			  			      paste( size_x, "x", size_y, sep="" ) ) 
		
		create_directory_if_not_exists(soms_results_directory)
		...
		saveRDS( soms_results, 
		 			  file=file.path(soms_results_directory,
		...
		draw_soms_xyplot_pdf ( soms_results$pivot, size_x, size_y, soms_results_directory  ) 
		...
		enriched_genes_file_name <- file.path( soms_results_directory, "enriched_genes_SOMs.txt" )
		enriched_go_terms_file_name <- file.path( soms_results_directory, "enriched_go_terms_SOMs.txt" )
		
		updated_enriched_genes <- go_enrichment_results$enriched_genes %>%
		                          left_join( gene_annotation_table, by=c("gene_id" = "orf_id")) 
	...
	#                                                     "blast2go_annotation", "is_tf", "is_kinases" )))
		...
		write.table (  go_enrichment_results$enriched_go_terms,  enriched_go_terms_file_name, sep="\t", row.names = FALSE, quote=FALSE)
		...
		write.table( directionality_of_each_cluster, file.path( soms_results_directory, "soms_clusters_mapping.txt"), sep="\t",
...
# size_x <- 6
# size_y <- 7
# results_directory <- paste( EdgeR_RUVs_QL_soms_results_dir, "_testing", sep="")
# 
...
updated_all_time_points <- updated_all_time_points[updated_all_time_points != "AL2"]
...
list_to_redraw_soms <- list( pivoted_table=list_of_clusters, size_x=list_of_x, size_y=list_of_y,
							soms_results_directory=directory_cleaning)

purrr::map ( list_to_redraw_soms, length)

purrr::pwalk ( list_to_redraw_soms, 
			   function ( pivoted_table, size_x, size_y, soms_results_directory) {
			
			   	my_pdf_file <- file.path(soms_results_directory, 
				...
				dev.off() 
				 ...
				 my_updated_pivot_file <- file.path(soms_results_directory, 
...
```{r}
# compare_A_soms <- read.table(file.path(results_directory, "compare_A/5x5/updated_SOMs_data_5_by_5.txt"), header=TRUE,
...
purrr::walk ( list_of_files, 
            ...
            genes_vs_go_terms <-  read.table( x, header=TRUE, sep="\t", stringsAsFactors = FALSE, quote="")
            ...
            enriched_genes_soms_cleaned <- genes_vs_go_terms %>%
                               dplyr::left_join( list_of_go_terms, by = "go_id" ) %>%
	                             dplyr::select( one_of( c( "go_id", "go_term", "gene_id", "ontology",
	                                                       "annotation_list_id", "query_list_id", "organism",
	                                                       "gene_names",	"uniprot_acc",	"broad_gene_name",	"uniprot_gene_name",	
                                                         "fungidb_gene_name",	"blast2go_annotation",	"is_tf",	"is_kinases",	
                                                         "is_virulence_gene",	"order_peaktime",	"scer_orthologs",	"scer_ortholog_descriptions" )))
