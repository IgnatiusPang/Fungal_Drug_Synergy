...
base_results_dir <-    file.path( results_dir, "Differential_Expression/EdgeR_RUVs_QL/cneo/To_Use_k_0_Remove_5a" ) 

##### This is a parameter to change #### *******************
de_method_label <-   "To_Use_k_0_Remove_5a" 

####################################################

specific_subset <-  "AMB only" # "AMB+LF only" #
...
if( specific_subset == "AMB only") {
  
  go_terms_results_dir <- file.path( results_dir, "Functional_Enrichment", de_method_label,"GOStat_AMB_only_set")
  
} else if ( specific_subset == "AMB+LF only") {
  
  go_terms_results_dir <- file.path( results_dir, "Functional_Enrichment", de_method_label,"GOStat_AMB_LF_set")
} else {
  stop("Not the correc specific subset.")
}
...
go_dictionary <- get_crypto_go_dictionary_of_choice  ( "Cryptococcus neoformans H99", gene_ontology_dir, 
									  bp_file_name="cneo_goframeData_bp.tab", 
									  mf_file_name="cneo_goframeData_mf.tab", 
									  cc_file_name="cneo_goframeData_cc.tab"  ) 

gene_annotation <- read.table ( file.path( data_dir, 'Gene_Annotation/cneo_gene_annotation_summary.txt' ), 
...
## Get list of differentially expressed genes specific to AMB+LF treatment 
```{r}
...
compare_log_fold_change <- read_tsv( file.path( results_dir, "Compare_Fold_Change/compare_A_AL_fold_change.tab") ) 

compare_specific_set.up <- compare_log_fold_change %>% 
  filter ( coarse_groups == specific_subset) %>%
  filter ( logFC.AL > 0 ) %>%
  pull( genes)

compare_specific_set.down <-  compare_log_fold_change %>% 
  filter ( coarse_groups == specific_subset) %>%
  filter ( logFC.AL < 0 ) %>%
  pull( genes) 
...
if( specific_subset == "AMB+LF only") {
  
  list_of_up_and_down_genes <- list( compare_AL_specific.up = compare_specific_set.up, 
                                   compare_AL_specific.down = compare_specific_set.down)

} else {
  list_of_up_and_down_genes <- list( compare_A_specific.up = compare_specific_set.up, 
                                   compare_A_specific.down = compare_specific_set.down)
...
go_enrichment_results$enriched_genes <- go_enrichment_results$enriched_genes %>%
										...
										dplyr::left_join( gene_annotation , by=c("gene_id"="orf_id")) %>%
...
columns_to_include <-   c( "query_list_id", "rank", "go_id",  "ontology", "term", "bh", "gene_id", "gene_name", "logFC",  FDR_column_name, 
  ...
  "is_tf", "is_kinases",                 
 "is_virulence_gene", "order_peaktime", "scer_orthologs", "scer_ortholog_descriptions", "expt_label" ) 
