...
compare_AL_specific.up <- compare_log_fold_change %>% 
  filter ( coarse_groups == "AMB+LF only") %>%
  filter ( logFC.AL > 0 ) %>%
  pull( genes)

compare_AL_specific.down <-  compare_log_fold_change %>% 
  filter ( coarse_groups == "AMB+LF only") %>%
  filter ( logFC.AL < 0 ) %>%
  pull( genes) 

list_of_up_and_down_genes <- list( compare_AL_specific.up = compare_AL_specific.up, 
                                   compare_AL_specific.down = compare_AL_specific.down)
