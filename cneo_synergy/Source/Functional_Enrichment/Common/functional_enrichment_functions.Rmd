...
get_crypto_go_dictionary_of_choice <- function (  organism_long_name, annotation_dir, bp_file_name, mf_file_name, cc_file_name) {

        ### Using Blast2GO, Uniprot, and manual OrthoMCL (1 to 1 matches) annotations
        goframeData_bp <- read.table ( file.path( annotation_dir, bp_file_name),  sep="\t", header=TRUE) 
        goframeData_mf <- read.table ( file.path( annotation_dir, mf_file_name),  sep="\t", header=TRUE) 
        goframeData_cc <- read.table ( file.path( annotation_dir, cc_file_name),  sep="\t", header=TRUE) 
...
### Function: get_cneo_universe
Get the list of all genes for C. neoformans H99
```{r}

get_cneo_universe <- function(my_channel) {
  # List of all the genes from C. neoformans H99
        ### Need to replace query of list of universe genes with a proper one
        universe_helper <- sqlQuery( my_channel, " select distinct orf_id from genome_annotation_cneo_table order by orf_id")
...
* results\_dir - the directory to save the output files 
* pvalueCutoff  - Benjamini-Hochberg adjusted p-value < X for a GO term to be significant (default 0.05)
