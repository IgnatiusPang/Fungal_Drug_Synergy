...
```{r}
read_counts_files <- list.files (scer_original_read_counts_dir, pattern="_featurecounts.txt$" )
...
list_of_read_counts_table <- purrr::map ( read_counts_files , function(x) {  read.table (file.path( scer_original_read_counts_dir, x ), header=TRUE)}  )
...
clean_col_names <- function(.x){ 
            case_when( str_detect(.x, "scer_AMB_only_control_rep") ~ str_replace(.x, "scer_AMB_only_control_rep", "CA_"),
           str_detect(.x, "scer_AMB_only_rep") ~ str_replace(.x, "scer_AMB_only_rep", "A_"),
           str_detect(.x, "scer_AMB_LF_control_rep") ~ str_replace(.x, "scer_AMB_LF_control_rep", "CAL_"),
           str_detect(.x, "scer_AMB_LF_rep") ~ str_replace(.x, "scer_AMB_LF_rep", "AL_"),
...
write.table( read_counts_with_columns, file=scer_read_counts_file, 
...
read_counts_summary_files <- list.files (scer_original_read_counts_dir, pattern="_featurecounts.txt.summary$" )
...
list_of_summary_table <- purrr::map ( read_counts_summary_files , 
                               function(x) {  read.table (file.path( scer_original_read_counts_dir, x ), header=TRUE)}  )
...
write.table( read_counts_summary_with_row_id, file=file.path( read_counts_directory, "scer/scer_synergy_read_counts_summary_concat.txt" ), 
			 quote=FALSE, sep="\t", row.names = FALSE, col.names = TRUE)
