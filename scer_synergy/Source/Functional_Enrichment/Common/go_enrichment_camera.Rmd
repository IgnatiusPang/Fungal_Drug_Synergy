...
camera_go_terms_results_dir <- file.path( results_directory, "Functional_Enrichment", de_method_label, "Camera")

# No need to change this parameter
create_directory_if_not_exists(camera_go_terms_results_dir)
...
# write.table ( logCPM, file=file.path( rna_seq_results_directory, "log_normalised_counts_after_calcNormFactors.txt" ), col.names = TRUE, row.names = TRUE)
...
format_go_dictionary_for_camera_analysis <- function( goframeData, normalized_read_counts ) { 
	...
	list_of_gene_sets_v2 <- goframeData %>%
							dplyr::filter( oln_id %in% rownames( normalized_read_counts)  )  %>% ## Only get GO terms for genes of relevance
	 						dplyr::select( one_of( c("go_id", "oln_id"))) %>%
							dplyr::group_by (go_id) %>%
							dplyr::summarise ( gene_set  = list(purrr::reduce( as.character(oln_id), c)) ) %>%
...
```{r}
go_bp_file_name  <- "scer_goframeData_bp.tab"
go_bp_frameData <- read.table ( file.path( gene_ontology_directory, go_bp_file_name),  sep="\t", header=TRUE, stringsAsFactors = FALSE) 
head( go_bp_frameData)

go_cc_file_name  <- "scer_goframeData_cc.tab"
go_cc_frameData <- read.table ( file.path( gene_ontology_directory, go_cc_file_name),  sep="\t", header=TRUE, stringsAsFactors = FALSE) 


go_mf_file_name  <- "scer_goframeData_mf.tab"
go_mf_frameData <- read.table ( file.path( gene_ontology_directory, go_mf_file_name),  sep="\t", header=TRUE, stringsAsFactors = FALSE) 

deprecated_go_terms_file  <-  "go_ids_not_found.tab"
deprecated_go_terms <- read.table ( file.path( gene_ontology_directory, deprecated_go_terms_file),  
...
camear_results_genes_detailed <- camera_results_with_genes %>% 
								 ...
								 dplyr::left_join ( gene_annotation, by=c("list_of_de_genes" = "oln_id"))
