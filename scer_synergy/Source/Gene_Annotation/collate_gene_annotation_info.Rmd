...
cneo_gene_annotation <- read.table ( file.path( data_dir, 'Gene_Annotation/cneo_gene_names.txt' ) , 
								header=TRUE, sep="\t", quote="", stringsAsFactors = FALSE, na.strings = "NA")

scer_gene_annotation <- read.table ( file.path( data_dir, 'Gene_Annotation/scer_gene_names.txt' ) , 
...
orthologs_summary <- orthologs %>%
            ...
            dplyr::rename( cneo_uniprot_description = "uniprot_gene_name"  ) %>%
  					dplyr::group_by( cneo_orf_id, scer_oln_id) %>%
...
scer_gene_annotation_updated <- scer_gene_annotation %>% 
	...
	dplyr::left_join ( kinases_summary, by=c("scer_oln_id" = "oln_id")) %>%
		...
		dplyr::left_join( orthologs_summary, by =c("scer_oln_id" = "scer_oln_id") ) %>%
    dplyr::rename( oln_id = "scer_oln_id")

scer_gene_annotation_updated %>%
  filter (!is.na(cneo_orthologs) )


write.table ( scer_gene_annotation_updated, 
              file=file.path(data_dir, "Gene_Annotation", "scer_gene_annotation_summary.txt" ),
